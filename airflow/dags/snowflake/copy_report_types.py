...
def copy_report_types_table():
    ...
    @task
    def truncate_table():
        truncate_report_types = """
        USE ELECTION.RAW;

        TRUNCATE TABLE election.raw.RAW_REPORT_TYPES;
        """
        snowflake_query = SnowflakeOperator(
            task_id="snowflake_query",
            sql=truncate_report_types,
    ...
    def copy_table(params):
        ...
        COPY INTO election.raw.RAW_REPORT_TYPES
        FROM @REPORT_TYPES_STAGE/{run_date}_report_types.csv
        ON_ERROR = 'SKIP_FILE_5%'
        FILE_FORMAT = FEC_CSV;
        """
        snowflake_query = SnowflakeOperator(
            task_id="snowflake_query",
            sql=copy_report_types,
            snowflake_conn_id="snowflake_conn"
        )
        snowflake_query.execute(context={})
